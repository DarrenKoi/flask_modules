...
- **`ftp_handler/`** — equipment FTP collection at two scales. `FtpClient` (`ftp_client.py`) is the single-server primitive: one reused connection, context manager, four ad-hoc ops (`list_dir`, `download`, `upload`, `remove`); server errors propagate. `FtpFleetDownloader` (`ftp_fleet_downloader.py`) is the concurrent fan-out across the ~300-host fleet — `download` (in-memory or streaming via `on_file`) and `list_dirs` (discovery-only listing pass; `ListingReport.to_specs()` feeds the chosen paths back into `download`), both built on a shared `_run_fleet` engine (semaphore + pool + per-host timeout + failure isolation). `eqp_ftp_collect.py` is the archive→parse→index glue; `ftp_flask_proxy.py` / `ftp_flask_downloader.py` are the firewalled-client proxy pair (same API over HTTP). Tests patch `ftp_handler.ftp_client.FTP` / `ftp_handler.ftp_fleet_downloader.FTP`. See `ftp_handler/CONTEXT.md` for the glossary and `ftp_handler/adr/` for decisions.
