...
## Lifecycle을 활용한 retention

자세한 wrapper 사용법은 `usage.md`의 lifecycle 섹션을 참고. 이 문서에서는
**언제 어떤 정책이 어울리는지** 만 정리합니다.
...
기본 default는 "영구 보관 + lifecycle 미적용"이고, **명시적인 retention
요구사항이 있을 때만** lifecycle rule을 추가합니다. 자동 삭제는 일반적인
data loss bug보다 더 조용히 사고를 만듭니다.
...
`2067928/trash/`에 30일 lifecycle expiration 걸어 두면 자동 정리.
...
lifecycle filter로 `tag:deleted=true`인 객체만 30일 후 삭제. application은
list 시 tag로 필터하면 "현재 보이는 파일"만 표시.

prefix 이동이 더 단순하고, 권한/storage 분리에도 유리해 자주 쓰입니다.
...
- **무한 versioning.** versioning만 켜고 `noncurrent_version_expiration`을
  안 걸면 storage가 영원히 증가합니다. 켜는 순간 lifecycle도 함께 정의.
